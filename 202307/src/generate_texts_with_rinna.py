...
def generate_text_fn(model, tokenizer, args: dict = {}):
    def generate_text(prompt: str) -> str:
        input_tokens = tokenizer.encode(prompt.replace("\n", "<NL>"), add_special_tokens=False, return_tensors="pt").to(model.device)
        ...
        with torch.no_grad():
            ...
            output_tokens = outputs.tolist()[0][input_tokens.size(1):]

        return tokenizer.decode(output_tokens).replace("<NL>", "\n")
